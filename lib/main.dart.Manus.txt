...
import 'dart:math';
import 'dart:convert'; // Aggiunto per JSON encoding/decoding

void main() async {
  WidgetsFlutterBinding.ensureInitialized();
  await NotificationService().init();
  tzdata.initializeTimeZones();
  tz.setLocalLocation(tz.getLocation('Europe/Rome')); // Impostazione del fuso orario locale
  runApp(const MyApp());
}
...
class _AlarmGameScreenState extends State<AlarmGameScreen> {
  ...
  void _posticipaSveglia() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Sveglia posticipata di 5 minuti')),
    );
    Navigator.pop(context);
  }
  ...
  static MathProblem _mathProblem(Random random) {
    ...
    switch (operator) {
      ...
      case '*':
        ...
        break;
      case '/':
        // Corretto per usare la divisione invece della moltiplicazione
        num1 = num3 * num2; // Assicura che la divisione dia un risultato intero
        expression = '$num1 ÷ $num2';
        answer = num3.toString(); // Il risultato è num3
        break;
      default:
        expression = '$num1 + $num2';
        answer = (num1 + num2).toString();
...
class Reminder {
  ...
  factory Reminder.fromJson(String jsonStr) {
    ...
    return Reminder(
      ...
      endTime: (map['endTime'] != null && map['endTime'].toString().isNotEmpty)
